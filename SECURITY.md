...
| Self-update supply chain | `nostos update` issues a single HTTPS GET to api.github.com. The `--verify` flag runs `git verify-tag` on the release tag for source-clone installs; if the signature does not verify, the upgrade is refused (fail-closed, and `--yes` does not override it). `--offline` disables the network call entirely. pip installs are never upgraded automatically. |
